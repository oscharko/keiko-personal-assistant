...
from core.beta_auth import BetaAuthHelper
...
async def beta_auth_status():
    """Check if beta authentication is enabled"""
    beta_auth = current_app.config.get("BETA_AUTH_HELPER")
    return jsonify({
        "betaAuthEnabled": beta_auth.enabled if beta_auth else False
    })


@bp.route("/auth/login", methods=["POST"])
async def beta_auth_login():
    """Beta authentication login endpoint"""
    beta_auth = current_app.config.get("BETA_AUTH_HELPER")

    if not beta_auth or not beta_auth.enabled:
        return jsonify({"error": "Beta authentication is not enabled"}), 400
    ...
    try:
        ...
        if not beta_auth.validate_credentials(username, password):
        ...
        logging.exception("Error during beta auth login")
...
async def setup_clients():
    ...
    # Setup beta authentication
    beta_auth_enabled_str = os.getenv("BETA_AUTH_ENABLED", "false")
    beta_auth_enabled = beta_auth_enabled_str.lower() == "true"
    logging.info(f"BETA_AUTH_ENABLED environment variable: '{beta_auth_enabled_str}', parsed as: {beta_auth_enabled}")
    beta_auth_helper = BetaAuthHelper(enabled=beta_auth_enabled)
    logging.info(f"BetaAuthHelper initialized with enabled={beta_auth_helper.enabled}")
    current_app.config["BETA_AUTH_HELPER"] = beta_auth_helper
